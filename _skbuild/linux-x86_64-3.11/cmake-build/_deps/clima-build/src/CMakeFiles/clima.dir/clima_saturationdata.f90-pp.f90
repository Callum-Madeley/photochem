# 1 "/data/cjm267/photochem/_skbuild/linux-x86_64-3.11/cmake-build/_deps/clima-build/src/clima_saturationdata.f90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data/cjm267/photochem/_skbuild/linux-x86_64-3.11/cmake-build/_deps/clima-build/src/clima_saturationdata.f90"
...
module clima_saturationdata
  ...
  interface sat_pressure_vap
    module procedure :: sat_pressure_vap_real, sat_pressure_vap_dual
  end interface
  interface sat_pressure_sub
    module procedure :: sat_pressure_sub_real, sat_pressure_sub_dual
  end interface
  interface sat_pressure
    module procedure :: sat_pressure_real, sat_pressure_dual
  end interface
  interface integral_fcn
    module procedure :: integral_fcn_real, integral_fcn_dual
  end interface
  interface sat_pressure_crit
    module procedure :: sat_pressure_crit_real, sat_pressure_crit_dual
  end interface
...
contains
  ...
  !> This is non-physical. Its to approximate transitions in atmospheres
  !> between super-critical and sub-critical gases.
  function sat_pressure_crit_real(self, T) result(p_sat)
  ...
  function sat_pressure_vap_real(self, T) result(p_sat)
    use clima_const, only: Rgas
    class(SaturationData), intent(inout) :: self
    real(dp), intent(in) :: T !! K
    real(dp) :: p_sat !! dynes/cm2
  ...
  function sat_pressure_sub_real(self, T) result(p_sat)
    use clima_const, only: Rgas
    class(SaturationData), intent(inout) :: self
    real(dp), intent(in) :: T !! K
    real(dp) :: p_sat !! dynes/cm2
  ...
  function sat_pressure_real(self, T) result(p_sat)
    class(SaturationData), intent(inout) :: self
    real(dp), intent(in) :: T !! K
    real(dp) :: p_sat !! dynes/cm2
    if (T >= self%T_critical) then
  ...
  function integral_fcn_real(A, B, T) result(res)
  ...
  !> This is non-physical. Its to approximate transitions in atmospheres
  !> between super-critical and sub-critical gases.
  function sat_pressure_crit_dual(self, T) result(p_sat)
    use forwarddiff
    use clima_const, only: Rgas
    class(SaturationData), intent(inout) :: self
    type(dual), intent(in) :: T !! K
    type(dual) :: p_sat !! dynes/cm2
    type(dual) :: tmp
    tmp = (integral_fcn(self%a_v, self%b_v, self%T_critical) - integral_fcn(self%a_v, self%b_v, self%T_ref)) + &
          (integral_fcn(self%a_c, self%b_c, T) - integral_fcn(self%a_c, self%b_c, self%T_critical))
    p_sat = self%P_ref*exp((self%mu/Rgas)*(tmp))
  ...
  !> Saturation pressure over liquid
  function sat_pressure_vap_dual(self, T) result(p_sat)
    use forwarddiff
    use clima_const, only: Rgas
    class(SaturationData), intent(inout) :: self
    type(dual), intent(in) :: T !! K
  ...
  !> Saturation pressure over solid
  function sat_pressure_sub_dual(self, T) result(p_sat)
    use forwarddiff
    use clima_const, only: Rgas
    class(SaturationData), intent(inout) :: self
    type(dual), intent(in) :: T !! K
  ...
  !> Saturation pressure over liquid or solid
  function sat_pressure_dual(self, T) result(p_sat)
    use forwarddiff
    class(SaturationData), intent(inout) :: self
    type(dual), intent(in) :: T !! K
    type(dual) :: p_sat !! dynes/cm2
  ...
  !> This is $\int L/T^2 dT$
  function integral_fcn_dual(A, B, T) result(res)
    use forwarddiff
    real(dp), intent(in) :: A, B
    type(dual), intent(in) :: T !! K
    type(dual) :: res
