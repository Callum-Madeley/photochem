# 1 "/data/cjm267/photochem/_skbuild/linux-x86_64-3.12/cmake-build/_deps/clima-build/src/dependencies/linear_interpolation_module.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data/cjm267/photochem/_skbuild/linux-x86_64-3.12/cmake-build/_deps/clima-build/src/dependencies/linear_interpolation_module.F90"
!*****************************************************************************************
...
!  1D linear interpolation routine.
    pure subroutine interp_1d_real(me,x,f,istat)
    ...
    pure subroutine interp_1d_dual(me,x,f,istat)
    use forwarddiff
    ...
    type(dual),intent(out)                  :: f     !! Interpolated \( f(x) \)
    integer,intent(out),optional          :: istat !! `0`  : no problems,
                                                   !! `-1` : class has not been initialized
...
!  * Jacob Williams, 10/9/2019 : added optional `inearest` output.
    pure subroutine dintrv_real(xt,x,ilo,ileft,iright,mflag,inearest)
    ...
    pure subroutine dintrv_dual(xt,x,ilo,ileft,iright,mflag,inearest)
    use forwarddiff

    implicit none

    real(wp),dimension(:),intent(in) :: xt       !! a knot or break point vector
    type(dual),intent(in)              :: x        !! argument
    integer,intent(inout)            :: ilo      !! an initialization parameter which must be set
                                                 !! to 1 the first time the array `xt` is
                                                 !! processed by dintrv. `ilo` contains information for
                                                 !! efficient processing after the initial call and `ilo`
